...
// Sidan låg på roten fram till vändningen och ärvde då layoutens metadata.
// Som undersida gör den inte det: filkonventionens delningsbild gäller bara
// det segment den ligger i, så utan den här raden blir /fynd en delad länk
// utan bild — och ingenting säger ifrån, eftersom en uteblivande metatagg
// inte går sönder.
export const metadata = sidmetadata({
  titel: 'Fem fynd',
  beskrivning:
    'Vad 2 569 voteringar med namnupprop tillsammans visar om mandatperioden 2022–2026: samstämmighet, ensamma linjer, frånvaro och de gånger kammaren fällde utskottet.',
  sokvag: '/fynd',
})
  ...
  return (
    <main>
      ...
      <section className="pb-16 pt-[72px] sm:pb-[72px] sm:pt-[88px]">
        ...
        >
          Här är fem saker som {heltal(d.voteringar)} voteringar med namnupprop
          tillsammans visar. Varje tal går att följa tillbaka till besluten det
          är räknat på.
        </p>

        <div className="stig mt-10 flex flex-wrap gap-3" style={{ animationDelay: '160ms' }}>
          <Knapp href="#fynd">Läs de fem fynden</Knapp>
          <Knapp href="/" ton="sekundar">Sök bland besluten</Knapp>
        </div>
      </section>
      ...
      {/* De fyra genomgångarna. De låg i navigeringen fram till vändningen och
          nås nu härifrån — sidorna finns kvar oförändrade på sina adresser. */}
      <section className="regel py-16">
        <h2 className="rubrik text-[clamp(2rem,5vw,44px)]">Hela genomgången</h2>
        <p className="mt-5 max-w-[62ch] text-[16.5px] leading-[1.6]" style={{ color: 'var(--black-mjuk)' }}>
          Fynden ovan är sammandraget. Varje mått finns utskrivet i sin helhet,
          med underlaget och invändningarna redovisade.
        </p>
        <ul className="mt-9">
          {[
            { href: '/samstammighet', rubrik: 'Vem röstar med vem',
              text: 'Matrisen med alla 28 partipar, och vad "samma linje" betyder.' },
            { href: '/amnen', rubrik: 'Var är riksdagen oenig',
              text: `Enigheten ämne för ämne, alla ${AMNEN.length} mätta likadant.` },
            { href: '/partier', rubrik: 'Partierna',
              text: 'Åtta sidor: samstämmighet, ensamma linjer, frånvaro och reservationer.' },
            { href: '/blocken', rubrik: 'Blocken',
              text: 'Tre oberoende mått på att riksdagen delade sig, med fyra invändningar prövade.' },
            { href: '/franvaro', rubrik: 'Frånvaron',
              text: 'Alla fyra riksmöten, och de voteringar frånvaron kunde ha avgjort.' },
          ].map((l) => (
            <li key={l.href} className="regel">
              <Link
                href={l.href}
                className="flex flex-wrap items-baseline gap-x-6 gap-y-1.5 py-5 transition-opacity duration-150 hover:opacity-70"
              >
                <span className="text-[19px] font-bold tracking-[-0.01em]">{l.rubrik}</span>
                <span className="text-[15px]" style={{ color: 'var(--black-svag)' }}>{l.text}</span>
              </Link>
            </li>
          ))}
        </ul>
